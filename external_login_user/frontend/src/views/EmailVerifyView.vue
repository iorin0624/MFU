...
async function verify() { busy.value = true; try { await portalApi.verifyEmail(pin.value); await store.bootstrap(true); await router.push('/'); } catch (e) { message.value = e instanceof Error ? e.message : '確認できません。'; } finally { busy.value = false; } }
</script>
