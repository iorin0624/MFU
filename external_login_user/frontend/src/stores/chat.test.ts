...
const mocks=vi.hoisted(()=>({snapshot:vi.fn(),dmSnapshot:vi.fn(),emit:vi.fn(),read:vi.fn(),unread:vi.fn(),leave:vi.fn(),socket:{connected:true}}));
vi.mock('@/api/client',()=>({setChatCsrfToken:vi.fn(),portalApi:{chatEventSnapshot:mocks.snapshot,chatDmSnapshot:mocks.dmSnapshot,markChatRoomNotificationsRead:mocks.read,chatRoomUnread:mocks.unread,chatPresence:mocks.leave}}));
vi.mock('@/services/portalRealtime',()=>({emitPortalEvent:mocks.emit,portalSocket:()=>mocks.socket,onPortalConnection:()=>()=>{},onPortalEvent:()=>()=>{},onPortalResume:()=>()=>{}}));
