...
def _create_payment_request(event_id: int, user_id: int, amount_yen: int) -> str:
    """支払いリクエストを発行し、トークンを返す。"""
    token = str(uuid.uuid4())
    db = get_db(); cur = db.cursor()
    try:
        cur.execute("""
            INSERT INTO mfu_payment_request (token, event_id, user_id, amount_yen)
            VALUES (%s, %s, %s, %s)
        """, (token, event_id, user_id, int(amount_yen)))
        db.commit()
    finally:
        try: cur.close(); db.close()
        except Exception: pass
...
def pay_start(event_uuid: str):
    ...
    if request.method == "POST":
        ...
        if picked == "card":
            ...
            payment_token = _create_payment_request(ev["id"], me["id"], int(fee))  # type: ignore
            session["pay_ctx"] = {
                "mfu_event_id": ev["id"],
