...
def ensure_feature_access_schema() -> None:
    ...
    with _SCHEMA_LOCK:
        ...
        try:
            ...
            cur.executemany(
                """
                INSERT INTO mfu_features (feature_key, label, is_enabled_global)
                VALUES (%s, %s, %s)
                ON DUPLICATE KEY UPDATE
                    label=VALUES(label),
                    is_enabled_global=VALUES(is_enabled_global)
                """,
                _FEATURE_SEED,
            )
            cur.execute("SELECT COUNT(*) FROM mfu_nav_items")
            row = cur.fetchone()
...
def _seed_nav_items(cur) -> None:
    cur.execute(
        ...
        VALUES (%s, %s, %s, %s, %s)
        """,
        ("⬆️ アップロード関係", "/", 10, 1, "upload"),
    )
    upload_parent_id = cur.lastrowid
    cur.executemany(
        """
        ...
        """,
        [
            ("⚙️ モード管理", "/modes", upload_parent_id, 10, 1, "upload"),
            ("📜 アップロード履歴", "/upload_list", upload_parent_id, 20, 1, "upload"),
        ],
    )
    ...
    cur.executemany(
        """
        INSERT INTO mfu_nav_items
        (label, url, order_no, is_enabled, feature_key)
        VALUES (%s, %s, %s, %s, %s)
        """,
        [
            ("🖼 アルバム管理", "/album/admin_create_album/", 20, 1, "albums"),
            ("🎟 チケット管理", "/tickets/admin", 30, 1, "events"),
