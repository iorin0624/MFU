...
# --- 公開: Googleフォーム事前入力（UUID + UA） ---
from urllib.parse import quote_plus  # 既にあれば重複可

GOOGLE_FORM_BASE = (
    "https://docs.google.com/forms/d/e/1FAIpQLSdapxT1CQf9cORdmtV2z3EXCW_4E2ArcVxz2ZODEzyyD32sxA/viewform"
)
GOOGLE_FORM_UUID_ENTRY_ID = "547803822"  # entry.<ID> = チケットUUID
GOOGLE_FORM_UA_ENTRY_ID   = "313155255"  # entry.<ID> = User-Agent（UA環境）

# 公開プレフィックス配下（未ログインでもOKな帯域）に配置
@tickets_bp.route("/tickets/public/form/<uuid>", endpoint="ticket_contact_form_public")
@tickets_bp.route("/t/<uuid>/contact",          endpoint="ticket_contact_form_short")
def ticket_contact_form(uuid: str):
    """
    問い合わせ: チケットUUIDとUAをGoogleフォームの事前入力に埋め込んで302リダイレクト。
    ※ 公開期間外（開始前/終了後）でもフォームへ飛ばす。
    """
    # UUIDの存在だけはチェック（不正UUIDは404）
    ctx = _load_ticket_context(uuid)
    if not ctx:
        abort(404)

    # ここでは _batch_guard(ctx) の結果で 403 を返さない（問い合わせは常に可）
    # msg = _batch_guard(ctx)
    # if msg:
    #     return jsonify({"ok": False, "error": "forbidden", "message": msg}), 403
    ...
    q = (
        f"usp=pp_url"
        f"&entry.{GOOGLE_FORM_UUID_ENTRY_ID}={quote_plus(uuid)}"
        f"&entry.{GOOGLE_FORM_UA_ENTRY_ID}={quote_plus(ua)}"
    )
    url = f"{GOOGLE_FORM_BASE}?{q}"
