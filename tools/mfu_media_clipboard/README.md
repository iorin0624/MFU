...
タスクトレイの「URLを指定して取得」では、Instagram / Threads / X のURLを改行区切りで最大20件まで指定できます。ダイアログを開いている間はクリップボードを監視し、対象ドメインのURLを重複させず自動追記します。URLの投稿形式はクライアント側で限定せず、対応メディアはサーバー側で判定します。複数URLはサーバー負荷を抑えるため1件ずつ順番に取得し、各URLの選択・保存が終わると次へ進みます。
