...
セッションの `last_seen_at` は5分以上経過した場合だけ条件付きで更新します。これは監査用の
ベストエフォート記録であり、同時更新や一時的なDBエラーで失敗しても、直前の読み取りで有効性を
確認できたAPIリクエストは失敗させません。失効・有効期限・利用者状態の検証は毎回行います。
...
GET    /api/v1/calendar?year=&month=&season_id=
