...
type Entry={user_public_id:string;display_name:string;park?:string;arrival_time?:string;costume?:string;memo?:string}
...
<template><section class="panel calendar-panel"><h1>統合カレンダー</h1><p v-if="error" class="error">{{error}}</p><form class="calendar-filters" @submit.prevent="load"><label class="field">シーズン<select v-model="seasonId"><option v-for="season in seasons" :key="season.public_id" :value="season.public_id">{{season.name}}</option></select></label><label class="field">月<input v-model="month" type="month"></label><label class="field">利用者ID（任意）<input v-model="personId" maxlength="26"></label><label class="field-check"><input v-model="myselfOnly" type="checkbox">自分の予定だけ</label><button class="button">表示</button></form><h2>{{title}}</h2><div class="calendar-days"><button v-for="day in days" :key="day.date" class="calendar-day" @click="selected=day"><strong>{{day.date}}</strong><span>{{day.count}}人</span><small>ランド {{day.land_count}} / シー {{day.sea_count}}</small></button><p v-if="!days.length" class="helper">この月に表示できる予定はありません。</p></div><div v-if="selected" class="visit-list"><h2>{{selected.date}}の予定</h2><article v-for="entry in selected.entries" :key="entry.user_public_id" class="visit-card"><h3>{{entry.display_name}}</h3><p v-if="entry.park">{{entry.park}}</p><p v-if="entry.arrival_time">到着 {{entry.arrival_time}}</p><p v-if="entry.costume">服装 {{entry.costume}}</p><p v-if="entry.memo">{{entry.memo}}</p></article></div></section></template>
