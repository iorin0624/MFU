...
const selectedDate = ref(formatDate(initialDate))
const days = ref<Day[]>([])
...
const cells = computed<Cell[]>(() => {
  ...
  return Array.from({ length: total }, (_, index) => {
    ...
    return { key: formatDate(value), date: formatDate(value), number, weekday: value.getDay() }
  })
})
...
function isToday(date: string) { return date === formatDate(today) }
...
async function loadCalendar() {
  ...
  try {
    const result = await api<{ days: Day[] }>(`/calendar?${new URLSearchParams({ year, month })}`)
    days.value = result.days; error.value = ''
  } catch (cause) {
...
async function moveMonth(offset: number) {
  ...
  selectedDate.value = formatDate(cursor.value)
...
async function goToday() {
  ...
  await selectDate(formatDate(today)); await loadCalendar()
}
onMounted(async () => {
  try {
  ...
  <section v-else-if="authenticated" class="home-calendar">
    ...
    <div class="month-calendar" aria-label="月間カレンダー">
      ...
      <template v-for="cell in cells" :key="cell.key">
        ...
        <button v-else class="calendar-cell" :class="{ selected: selectedDate === cell.date, today: isToday(cell.date), sunday: cell.weekday === 0, saturday: cell.weekday === 6 }" type="button" @click="selectDate(cell.date)">
    ...
    <section v-if="selectedDate" class="selected-date-panel">
      <div class="selected-date-heading"><h2>{{ selectedDate }}の予定</h2><RouterLink class="button" :to="addLink">この日に予定を追加</RouterLink></div>
