...
async function follow(person: Person) {
  try { await api(`/follows/${person.public_id}`, { method: 'POST' }); notice.value = `${person.display_name}さんをフォローしました。`; if (found.value?.public_id === person.public_id) await findPerson(); await load() }
  catch (value) { error.value = message(value) }
}
async function unfollow(person: Person) {
  try { await api(`/follows/${person.public_id}`, { method: 'DELETE' }); notice.value = 'フォローを解除しました。'; if (found.value?.public_id === person.public_id) await findPerson(); await load() }
  catch (value) { error.value = message(value) }
}
async function block(person: Person) {
  if (!confirm(`${person.display_name}さんをブロックしますか？ 双方のフォローも解除されます。`)) return
  try { await api(`/blocks/${person.public_id}`, { method: 'POST' }); if (found.value?.public_id === person.public_id) found.value = null; notice.value = 'ブロックしました。'; await load() }
  catch (value) { error.value = message(value) }
}
...
<template>
  <section class="panel">
    ...
    <article v-if="found" class="visit-card"><h2>{{ found.display_name }}</h2><p>つながりID {{ displayConnectionId(found.connection_id) }}</p><p v-if="found.follows_me">あなたをフォローしています</p><div class="actions"><button v-if="!found.following" class="button" @click="follow(found)">フォローする</button><button v-else class="button secondary" @click="unfollow(found)">解除</button><button class="button danger" @click="block(found)">ブロック</button></div></article>
    <h2>フォロワー</h2><div class="visit-list"><article v-for="person in followers" :key="person.public_id" class="visit-card"><h3>{{ person.display_name }} <small v-if="person.mutual">相互フォロー</small></h3><p>つながりID {{ displayConnectionId(person.connection_id) }}</p><div class="actions"><button v-if="!person.following" class="button" @click="follow(person)">フォローする</button><button v-else class="button secondary" @click="unfollow(person)">解除</button><button class="button danger" @click="block(person)">ブロック</button></div></article><p v-if="!followers.length" class="helper">フォロワーはいません。</p></div>
    <h2>フォロー中</h2><div class="visit-list"><article v-for="person in follows" :key="person.public_id" class="visit-card"><h3>{{ person.display_name }} <small v-if="person.mutual">相互フォロー</small></h3><p>つながりID {{ displayConnectionId(person.connection_id) }}</p><button class="button secondary" @click="unfollow(person)">解除</button></article><p v-if="!follows.length" class="helper">フォロー中の利用者はいません。</p></div>
