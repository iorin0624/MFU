...
async function follow(person: Person) {
  try { await api(`/follows/${person.public_id}`, { method: 'POST' }); notice.value = `${person.display_name}さんを登録しました。`; if (found.value?.public_id === person.public_id) await findPerson(); await load() }
  catch (value) { error.value = message(value) }
}
...
<template>
  <section class="panel">
    ...
    <h2>あなたを登録している人</h2><div class="visit-list"><article v-for="person in followers" :key="person.public_id" class="visit-card"><h3>{{ person.display_name }} <small v-if="person.mutual">相互登録</small></h3><p>つながりID {{ displayConnectionId(person.connection_id) }}</p><button v-if="!person.mutual" class="button" @click="follow(person)">登録する</button></article><p v-if="!followers.length" class="helper">あなたを登録している利用者はいません。</p></div>
