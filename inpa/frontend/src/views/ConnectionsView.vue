...
<template>
  <section class="panel">
    ...
    <article v-if="found" class="visit-card"><h2><RouterLink class="person-link" :to="`/people/${found.public_id}`">{{ found.display_name }}</RouterLink></h2><p>つながりID {{ displayConnectionId(found.connection_id) }}</p><p v-if="found.follows_me">あなたをフォローしています</p><div class="actions"><button v-if="!found.following" class="button" @click="follow(found)">フォローする</button><button v-else class="button secondary" @click="unfollow(found)">解除</button><button class="button danger" @click="block(found)">ブロック</button></div></article>
