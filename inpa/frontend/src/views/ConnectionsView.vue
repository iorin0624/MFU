...
type Person = { public_id: string; display_name: string; mutual?: boolean; following?: boolean; follows_me?: boolean; x_handle?: string; instagram_handle?: string }
const publicId = ref(''); const found = ref<Person | null>(null)
...
async function findPerson() {
  ...
  try { found.value = (await api<{ person: Person }>(`/people/${encodeURIComponent(publicId.value.trim())}`)).person }
  catch (value) { error.value = message(value) }
}
